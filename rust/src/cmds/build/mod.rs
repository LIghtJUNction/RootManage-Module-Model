...
// 常量定义
const SHELLCHECK_MAX_OUTPUT_SIZE: usize = 1024 * 1024; // 1MB
...
fn copy_update_json_to_dist(project_path: &Path) -> Result<()> {
    ...
    if update_json_path.exists() {
        ...
        let mut update_json: serde_json::Value = serde_json::from_str(&content)?;
        
        // 更新版本信息以匹配 module.prop
        if let Some(obj) = update_json.as_object_mut() {
            obj.insert("version".to_string(), serde_json::Value::String(project_info.version.clone()));
            if let Ok(version_code) = project_info.version_code.parse::<i64>() {
                obj.insert("versionCode".to_string(), serde_json::Value::Number(serde_json::Number::from(version_code)));
            }
        }
        
        // 写入更新后的 update.json 到分发目录
        let updated_content = serde_json::to_string_pretty(&update_json)?;
        fs::write(&dest_path, updated_content)?;
        
        println!("{} 复制并更新 update.json 到分发目录（版本: {}）", "[+]".green().bold(), project_info.version);
    }
...
fn check_shell_scripts(project_path: &Path, auto_fix: bool) -> Result<()> {
    ...
    // 检查是否安装了 shellcheck
    let shellcheck_available = check_shellcheck_availability();
    ...
    let mut all_fixes = String::new(); // 收集所有修复建议
    
    // 对每个 shell 脚本运行 shellcheck
    for sh_file in &sh_files {
        match analyze_single_script(sh_file, &mut report, &mut all_fixes) {
            Ok(script_has_errors) => {
                if script_has_errors {
                    has_errors = true;
                }
            }
            Err(e) => {
                println!("⚠️ 警告: 分析脚本 {} 时发生错误: {}", sh_file.display(), e);
            }
        }
    }
    ...
    let _ai_report_path = rmmp_dir.join("shellcheck.llms.txt");
    save_shellcheck_reports(&report, &rmmp_dir)?;
    
    // 处理自动修复建议
    handle_automatic_fixes(project_path, &rmmp_dir, &all_fixes, &sh_files, auto_fix)?;
...
/// 检查 shellcheck 是否可用
fn check_shellcheck_availability() -> bool {
...
/// 运行 shellcheck 分析单个文件
fn analyze_single_script(
    sh_file: &Path, 
    report: &mut ShellcheckReport,
    all_fixes: &mut String
) -> Result<bool> {
    const MAX_OUTPUT_SIZE: usize = SHELLCHECK_MAX_OUTPUT_SIZE;
    let mut has_errors = false;
    ...
    // 检查输出大小，防止内存耗尽
    if json_output.stdout.len() > MAX_OUTPUT_SIZE {
        println!("    ⚠️ 警告: shellcheck 输出过大 ({} bytes)，跳过详细分析", json_output.stdout.len());
        return Ok(false);
    }
    
    // 获取带 wiki 链接的详细输出（限制大小）
    ...
    if wiki_output.stdout.len() > MAX_OUTPUT_SIZE {
        println!("    ⚠️ 警告: shellcheck wiki输出过大，跳过");
    }
    
    // 获取 diff 格式的修复建议（限制大小）
    let diff_output = Command::new("shellcheck")
        .arg("--format=diff")
        .arg(&sh_file)
        .output()?;
        
    if diff_output.stdout.len() > MAX_OUTPUT_SIZE {
        println!("    ⚠️ 警告: shellcheck diff输出过大，跳过自动修复");
    }
...
/// 生成并保存检查报告
fn save_shellcheck_reports(report: &ShellcheckReport, rmmp_dir: &Path) -> Result<()> {
    // 写入 JSON 格式报告（机器友好）
    let json_report_path = rmmp_dir.join("shellcheck.json");
...
fn read_project_info(project_path: &Path) -> Result<ProjectInfo> {
    ...
    Ok(ProjectInfo { id, name, version, version_code, author })
}
...
fn copy_source_files(project_path: &Path, source_build_dir: &Path, rmake_config: &RmakeConfig) -> Result<()> {
    ...
    if let Some(src_config) = &rmake_config.build.src {        // 首先获取所有文件
...
/// 生成报告头部信息
fn generate_report_header(report: &ShellcheckReport) -> String {
...
/// 生成成功结果内容
fn generate_success_result() -> String {
    let mut content = String::new();
...
/// 按严重程度分组问题
fn group_issues_by_severity(issues: &[ShellcheckIssue]) -> (Vec<&ShellcheckIssue>, Vec<&ShellcheckIssue>, Vec<&ShellcheckIssue>, Vec<&ShellcheckIssue>) {
...
/// 生成问题详情内容
fn generate_issues_details(report: &ShellcheckReport) -> String {
    let mut content = String::new();
    let (errors, warnings, infos, styles) = group_issues_by_severity(&report.issues);
...
/// 生成建议内容
fn generate_recommendations(report: &ShellcheckReport) -> String {
    let mut content = String::new();
...
/// 生成快速修复指南
fn generate_quick_fix_guide() -> String {
    let mut content = String::new();
...
/// 重构后的AI友好报告生成函数（现在更短更清晰）
fn generate_ai_friendly_report(report: &ShellcheckReport) -> String {
    let mut content = String::new();
    
    // 添加报告头部
    content.push_str(&generate_report_header(report));
    
    if report.issues.is_empty() {
        // 成功情况
        content.push_str(&generate_success_result());
    } else {
        // 有问题的情况
        content.push_str(&generate_issues_details(report));
        content.push_str(&generate_recommendations(report));
        content.push_str(&generate_quick_fix_guide());
    }
...
/// 复制文件并规范化行结尾
fn copy_file_with_line_ending_normalization(src: &Path, dest: &Path) -> Result<()> {
    // 简单的文件复制，不进行行结尾规范化（为了测试简化）
...
fn try_git_apply(project_path: &Path, fixes_path: &Path) -> Result<()> {
    ...
    if output.status.success() {
        ...
        Err(anyhow::anyhow!("Git apply failed: {}", String::from_utf8_lossy(&output.stderr)))
    }
}
...
fn apply_fixes_directly(sh_files: &[PathBuf]) -> Result<usize> {
    // 简化实现：这里应该包含自动修复逻辑
    println!("{} 尝试自动修复脚本问题", "[fix]".yellow().bold());
...
fn recheck_fixed_scripts(sh_files: &[PathBuf]) -> Result<ShellcheckReport> {
    ...
    let mut all_fixes = String::new();
    
    for script_path in sh_files {
        let _ = analyze_single_script(script_path, &mut report, &mut all_fixes);
    }
